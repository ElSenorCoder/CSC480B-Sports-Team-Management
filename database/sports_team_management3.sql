...
SELECT 
    ...
    pu.phone AS emergency_contact_phone,
...
FROM users pu
INNER JOIN roles pr ON pu.role_id = pr.id AND pr.name = 'parent'
LEFT JOIN team_memberships tm ON 1=1
LEFT JOIN users pl ON tm.user_id = pl.id;
...
SELECT 
    ...
    COUNT(DISTINCT tm.user_id) AS total_rostered_players,
    COUNT(DISTINCT CASE WHEN g.game_date <= NOW() THEN g.id END) AS total_games_played,
    SUM(CASE 
        WHEN g.game_date <= NOW() AND g.home_team_id = t.id AND g.home_team_score > g.away_team_score THEN 1
        WHEN g.game_date <= NOW() AND g.away_team_id = t.id AND g.away_team_score > g.home_team_score THEN 1
        ELSE 0 
    END) AS wins,
    SUM(CASE 
        WHEN g.game_date <= NOW() AND g.home_team_id = t.id AND g.home_team_score < g.away_team_score THEN 1
    ...
    END) AS losses,
    SUM(CASE 
        WHEN g.game_date <= NOW() AND g.home_team_score = g.away_team_score THEN 1
        ELSE 0 
    END) AS ties,
    SUM(CASE 
        WHEN g.game_date <= NOW() AND g.home_team_id = t.id THEN g.home_team_score 
        WHEN g.game_date <= NOW() AND g.away_team_id = t.id THEN g.away_team_score 
        ELSE 0 
    END) AS points_scored,
    SUM(CASE 
        WHEN g.game_date <= NOW() AND g.home_team_id = t.id THEN g.away_team_score 
        WHEN g.game_date <= NOW() AND g.away_team_id = t.id THEN g.home_team_score 
        ELSE 0 
    END) AS points_allowed
FROM teams t
LEFT JOIN team_memberships tm ON t.id = tm.team_id AND tm.role_in_team = 'player'
